...
class Cluster:
    ...
    def container_dictionaries(self):
        ...
        return create_container_dict(
            self.park_pol_clean, self.district_pol, self.plot_pol_clean, self.building_breps_clean)
    
    def flatten_dictionary_list(self):
        """ dict to list for output visualization"""
        return geometry_flatten_lists(self.container_dictionaries, self.park_pol_clean)

    def geo_dictionary(self):
        """ dict to list for output visualization"""
        return geometry_lists(self.container_dictionaries, self.park_pol_clean)

    def compute_clustering_values(self):
        #return clustering_values(self.container_dictionaries)
        return normalized_clustering_dict(self.container_dictionaries)
